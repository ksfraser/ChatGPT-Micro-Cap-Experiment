...
class MarketFactorsController
{
     ...
     * GET /api/market-factors/indicator-accuracy
     */
    public function getIndicatorAccuracy(?string $indicator = null): array
    {
        try {
            if ($indicator) {
                $accuracy = $this->service->getIndicatorAccuracy($indicator);
                if ($accuracy === null) {
                    return [
                        'success' => false,
                        'error' => "No accuracy data found for indicator: $indicator"
                    ];
                }
                return [
                    'success' => true,
                    'data' => [$indicator => $accuracy]
                ];
            } else {
                $allAccuracy = $this->service->getAllIndicatorPerformance();
                return [
                    'success' => true,
                    'data' => $allAccuracy
                ];
    ...
    /**
     * Track a new indicator prediction
     * POST /api/market-factors/track-prediction
     */
    public function trackPrediction(): array
    {
        try {
            $input = json_decode(file_get_contents('php://input'), true);
            
            $required = ['indicator', 'symbol', 'prediction', 'confidence', 'current_price'];
     ...
     * PUT /api/market-factors/update-prediction/{predictionId}
     */
    public function updatePrediction(string $predictionId): array
    {
        try {
            $input = json_decode(file_get_contents('php://input'), true);
            ...
            $success = $this->service->updateIndicatorAccuracy(
                $predictionId,
                $input['outcome'],
     ...
     * POST /api/market-factors/weighted-score
     */
    public function calculateWeightedScore(): array
    {
        try {
            $input = json_decode(file_get_contents('php://input'), true);
            ...
            $result = $this->service->calculateWeightedScore(
                $input['symbol'],
    ...
    public function handleRequest(): void
    {
        ...
        try {
            ...
            } elseif ($method === 'GET' && $pathParts[2] === 'market-factors' && $pathParts[3] === 'indicator-accuracy') {
                $indicator = $_GET['indicator'] ?? null;
                $response = $this->getIndicatorAccuracy($indicator);
            } elseif ($method === 'POST' && $pathParts[2] === 'market-factors' && $pathParts[3] === 'track-prediction') {
                $response = $this->trackPrediction();
            } elseif ($method === 'PUT' && $pathParts[2] === 'market-factors' && $pathParts[3] === 'update-prediction' && isset($pathParts[4])) {
                $response = $this->updatePrediction($pathParts[4]);
            } elseif ($method === 'POST' && $pathParts[2] === 'market-factors' && $pathParts[3] === 'weighted-score') {
                $response = $this->calculateWeightedScore();
